...
Dnichenetwork<-function(scObject,groupby="cell_clusters",share_method="min",direction=FALSE){
  ...
  clone.size=sort(sapply(clone,function(x){nrow(x)}),decreasing = T)#correction1
  res[["clone"]]=clone
  res[["clone_size"]]=clone.size
  tag_clone=names(clone.size)[is.clone(names(clone.size))]#correction2
  tag_clone_ids=1:length(tag_clone)#correction2
  names(tag_clone_ids)=tag_clone#correction2
  res[["cloneID"]]=tag_clone_ids#correction2
...
#'
#' @param code vector, clone codes
#'
#' @returns TorF logic vector showing whether clones express tags
#' @importFrom stringr str_split_fixed
#' @export
#'
#' @examples
#' istagclone=is.clone(names(clone.size))
is.clone<-function(code){
  cell.cloneBarcode=str_split_fixed(code,"_",n=2)[,1]
  code_matrix=sapply(strsplit(cell.cloneBarcode, split = ""),as.integer)
